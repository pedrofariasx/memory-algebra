...
This proof was independently verified via categorical ground truth using Catlab.jl (Julia), confirming pushout isomorphism: \texttt{((m1⊕m2)⊕m3) ≅ (m1⊕(m2⊕m3))}.
...
The LSH pathway activates for $\geq 4096$ nodes, but vector allocation via \texttt{np.stack} and candidate pair generation still consume $O(N)$ memory. Scaling to $N = 10^4$ requires streaming or sparse storage optimizations.
...
\begin{enumerate}
    \item \textbf{Scale:} Current implementation is limited to $N \leq 10^3$ memories within 4\,GB RAM. Scaling to $10^4$+ requires sparse storage or streaming architectures.
    \item \textbf{Synthetic embeddings:} Validation uses controlled synthetic vectors. Real-world performance with sentence-transformer embeddings remains to be established.
...
Scalability to $N = 10^4$ remains an engineering challenge requiring sparse storage optimizations. The differentiable layer (JAX implementation) opens a path toward integration with neural learning while preserving algebraic guarantees.

\section*{Data and Code Availability}

All experimental data, analysis scripts, and implementation code are available in the project repository. Raw results are provided in JSON format (\texttt{experiments/results\_phase2.json}, \texttt{experiments/results\_phase3.json}).
