...
Esta prova foi independentemente verificada via ground truth categ\'orico usando Catlab.jl (Julia), confirmando isomorfismo de pushout: $((m_1 \oplus m_2) \oplus m_3) \cong (m_1 \oplus (m_2 \oplus m_3))$.
...
A Fase~4 tamb'em foi executada em escala: com $N = 10{,}000$ mem'orias reais ($\theta = 0{,}85$), a composi\c{c}\~ao completa leva 0,007\,s e a recupera\c{c}\~ao leva $\sim$20\,s, retornando 9.990 n'os. O tempo de recupera\c{c}\~ao reflete a alta densidade de pares similares intra-t'opico t'ipica de embeddings reais; a composi\c{c}\~ao permanece $O(N)$ e pr'atica mesmo em escala.
...
\begin{enumerate}
    ...
    \item \textbf{Escala do corpus real:} A Fase~4 valida embeddings reais em um corpus de 20 fatos. Valida\c{c}\~ao em larga escala com corpora reais permanece por ser estabelecida.
...
A valida\c{c}\~ao com embeddings reais de linguagem natural (Fase~4) confirma que associatividade, recuperabilidade e estabilidade se mant\^em fora do regime sint\'etico: 100\% de precis\~ao top-1 em recupera\c{c}\~ao por par\'afrase (20/20, score m\'edio 0,895).

Escalabilidade para $N = 10^4$ mem\'orias em 4\,GB de RAM \'e alcan\c{c}ada via LSH encadeado e composi\c{c}\~ao em lote (retrieve 0,478\,s); escalar al\'em de $10^4$ requer otimiza\c{c}\~oes de armazenamento esparso. A camada diferenci\'avel (implementa\c{c}\~ao JAX) abre caminho para integra\c{c}\~ao com aprendizado neural preservando as garantias alg\'ebricas.
